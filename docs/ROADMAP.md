...
### v0.5.0 - TTS模块（优先）
**状态**：✅ 已完成（GPT-SoVITS API 真实接入）

**功能清单**：
- [x] TTS框架搭建（策略模式 + ITTSProvider 接口）
- [x] ApiTTSProvider（GPT-SoVITS HTTP API 接入）
- [x] MockTTSProvider（模拟实现）
- [x] 双队列模型（合成队列 + 播放队列）
- [x] QMediaPlayer 音频播放集成
- [x] TTS 模式配置（api/mock 切换）
- [x] 错误处理（合成失败跳过，不阻塞队列）
- [x] 临时音频即用即删（播放完成后自动清理）

**技术任务**：
- [x] TTSService 重构为策略模式（ITTSProvider 抽象接口）
- [x] 集成 GPT-SoVITS HTTP API（POST /tts）
- [x] 集成 Qt Multimedia 音频播放
- [x] 参考音频情绪映射（7种情绪各对应一个 wav 文件）
- [x] 临时音频文件自动清理
...
**实现说明**：
- 音频合成后保存为临时文件，播放完成后立即删除
- 不实现音频缓存，每次合成都是新请求（简化设计）
- 合成失败时跳过当前句，不阻塞后续队列
...
| TD-005 | TTS为模拟实现 | 高 | v0.5.0 | ✅ 已修复（策略模式+GPT-SoVITS API接入完成） |
...
v0.5.0  ████████████████████ 100%  TTS模块（GPT-SoVITS API真实接入+策略模式）
v0.6.0  ░░░░░░░░░░░░░░░░░░░░   0%  设置界面 & 交互增强
v0.7.0  ░░░░░░░░░░░░░░░░░░░░   0%  架构改进 & 独立存档
v0.8.0  ░░░░░░░░░░░░░░░░░░░░   0%  角色自定义 & 剧本系统
v0.9.0  ░░░░░░░░░░░░░░░░░░░░   0%  智能助手 & 性能优化
...
2. **设置界面扩展** - 添加TTS配置页、外观配置、时间配置、快捷键配置
3. **代码质量提升** - 统一日志格式、添加函数注释、清理未使用代码
4. **长期记忆增强** - 历史记录导出功能、摘要重建机制
