...
- [x] 分层记忆系统
  ...
  - [x] 中期记忆（完整对话历史记录，SQLite）✅ 已实现（分页浏览、删除）
  - [ ] 长期记忆（重要事件摘要，跨会话记忆）
- [x] 对话历史查看界面 ✅ 已实现
  - [x] 历史记录列表（表格展示，分页浏览）✅
  - [x] 单条记录详情（悬停显示完整内容）✅
...
| TD-016 | AppController中魔法数字SUMMARY_THRESHOLD | 中 | v0.5.0 | 待修复 |
...
1. **架构改进** - 修复AnchorManager内存泄漏、配置化SUMMARY_THRESHOLD、优化LLMService依赖
2. **TTS真实接入** - 集成GPT-SoVITS推理引擎
3. **长期记忆** - 实现LLM摘要生成器，提取重要事件
4. **设置界面扩展** - 添加TTS配置页、外观配置、时间配置、快捷键配置
5. **代码质量提升** - 统一日志格式、添加函数注释、清理未使用代码
