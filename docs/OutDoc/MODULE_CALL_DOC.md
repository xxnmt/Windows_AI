...
| `version` | string | 否 | 模型版本：`v1` 或 `v2`，默认 `v2` |
| `t2s_weights_path` | string | 是 | GPT/T2S模型路径 |
| `vits_weights_path` | string | 是 | SoVITS/VITS模型路径 |
...
| 返回项 | 类型 | 说明 |
|--------|------|------|
| `sampling_rate` | int | 采样率（通常为32000） |
...
{
    "gpt_sovits_path": "D:\\Tools\\GPT_SoVITS_v3lora\\GPT-SoVITS-v3lora-20250228",
    "selected_gpt_model": "GPT_SoVITS/pretrained_models/s1bert25hz-2kh-longer-epoch=68e-step=50232.ckpt",
    "selected_sovits_model": "GPT_SoVITS/pretrained_models/s2G488k.pth",
...
4. **批量处理**：长文本使用 `batch_size > 1` 加速合成
